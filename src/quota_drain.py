...
if __name__ == "__main__":
    ...
    day_before = datetime.datetime.combine(
        date=today - datetime.timedelta(days=2), time=datetime.time(0, 0, 0)
    )
    offset = 0
    num = 100
    ...
    first_res = requests.get(
        ...
        params={
            "source-country": "in",
            "language": "en",
            "earliest-publish-date": day_before,
            "latest-publish-date": yesterday,
            "categories": "politics",
            "number": num,
            "offset": offset,
            "sort-direction": "DESC",
    ...
    while True:
        ...
        if float(next_res.headers.get("X-API-Quota-Left")) <= 0.0:
            break
        print(
            f"Received {offset} items so far... Quota left: {next_res.headers.get('X-API-Quota-Left')}"
        )
